	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i <= RT5647_VENDOR_ID2; i++) {
		...
		if (rt5647_readable_register(codec, i)) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < rt5647->adb_reg_num; i++) {
		...
		switch (rt5647->adb_reg_addr[i] & 0x10000) {
		case 0x10000:
...
static DEVICE_ATTR(codec_reg_adb, 0666, rt5647_codec_adb_show, rt5647_codec_adb_store);
	...
	case SND_SOC_BIAS_OFF:
		...
		if (rt5647->push_button_en) {
			...
			snd_soc_update_bits(codec, RT5647_PWR_ANLG2,
				RT5647_PWR_MB1, RT5647_PWR_MB1);
			snd_soc_update_bits(codec, RT5647_PWR_MIXER,
				RT5647_PWR_LDO2, RT5647_PWR_LDO2);
			snd_soc_update_bits(codec, RT5647_PWR_VOL,
				RT5647_PWR_MIC_DET, RT5647_PWR_MIC_DET);
		}
		break;
...
static void jd_check_handler(struct work_struct *work)
{
	...
	if (val == 0x20) {
		...
		rt5647_button_detect(codec);
		snd_soc_update_bits(codec, RT5647_INT_IRQ_ST, 0x8, 0x0);
		snd_soc_jack_report(rt5647_jack, 0, SND_JACK_BTN_0);
		rt5647_headset_detect(codec, 0);
		msleep(50);
		snd_soc_jack_report(rt5647_jack, 0, SND_JACK_HEADSET | SND_JACK_BTN_0);
		del_timer( &jd_check_timer);
	}
}
